...
API 키를 비워두면 AI 없이도 핵심 업무 관리 기능은 동작합니다. 기본 제공 엔진은 로컬 규칙이고, 로그인 후 `설정 → AI 서비스`에서 각 Google 계정별로 OpenAI 또는 Gemini를 선택해 키, 모델, 기준 URL을 따로 저장할 수 있습니다. `.env`의 AI 값은 계정별 설정이 없을 때의 서버 기본값으로만 쓰입니다.
...
로그인 후 `GET /api/ai/status` 또는 `GET /api/settings/ai`에서 실제 연결 설정을 확인할 수 있습니다. `configured`가 `false`이면 해당 계정에 저장된 키가 없어서 로컬 규칙 분석으로 자동 전환합니다. 분석 결과의 `source`가 `local-rules`이고 `warning`이 있으면 공급자 URL, 모델명, 결제 상태와 API 키 권한을 확인하세요. API 키 자체는 상태 응답에 노출되지 않습니다.
