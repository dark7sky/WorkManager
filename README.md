...
자연어 적용은 안전을 위해 **미리보기 한 건을 확인한 뒤 적용**합니다. 한 문장에 여러
생성·수정 요청이 있으면 첫 번째 동작만 미리보기합니다. 기존 항목 수정은 번호를 명시하세요
(예: `업무 #12 진행률 70%`).

이 배포판은 개인용 단일 테넌트입니다. ID/PW와 허용된 Google 계정은 같은 업무
데이터베이스를 공유합니다. 여러 사용자의 데이터를 분리해야 한다면 사용자 소유권과 권한
모델을 추가하기 전에는 배포하지 마세요.
