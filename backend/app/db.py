...
def init_db():
    with connection() as c:
        ...
        CREATE TABLE IF NOT EXISTS tasks(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id TEXT,title TEXT NOT NULL,description TEXT NOT NULL DEFAULT '',status TEXT NOT NULL DEFAULT 'todo',priority TEXT NOT NULL DEFAULT 'normal',progress INTEGER NOT NULL DEFAULT 0,start_date TEXT,due_date TEXT,tags TEXT NOT NULL DEFAULT '[]',recurrence_rule TEXT,recurrence_anchor_day INTEGER,recurrence_anchor_month_end INTEGER NOT NULL DEFAULT 0,parent_id INTEGER,dependency_ids TEXT NOT NULL DEFAULT '[]',recurrence_spawned_at TEXT,completed_at TEXT,deleted_at TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE CASCADE,FOREIGN KEY(parent_id) REFERENCES tasks(id) ON DELETE SET NULL);
        ...
        c.execute("INSERT INTO migration_state(key,value,updated_at) VALUES('schema_version','4',?) ON CONFLICT(key) DO UPDATE SET value='4',updated_at=excluded.updated_at", (datetime.now(timezone.utc).isoformat(),))
