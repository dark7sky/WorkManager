...
def google_start(request: Request):
    ...
    url = "https://accounts.google.com/o/oauth2/v2/auth?" + urlencode({"client_id": client_id, "redirect_uri": redirect_uri, "response_type": "code", "scope": "openid email profile", "state": state, "prompt": "select_account"})
    response = RedirectResponse(url)
    response.set_cookie("google_oauth_state", state, httponly=True, samesite="lax", max_age=600)
    return response
...
CONFIG = {
    ...
    "events": ({"title", "description", "start_at", "end_at", "location"}, None),
...
def create_item(table, data):
    ...
    if table == "tasks": data.update(created_at=timestamp, updated_at=timestamp)
    ...
    with connection() as c:
        ...
        return row_dict(c.execute(f"SELECT * FROM {table} WHERE id=?", (cur.lastrowid,)).fetchone())
...
def update_item(table, item_id, data):
    ...
    with connection() as c:
        ...
        return row_dict(c.execute(f"SELECT * FROM {table} WHERE id=?", (item_id,)).fetchone())
