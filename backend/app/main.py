...
def load_billing_biz_reg_number_setting(user_id):
    with connection() as c:
        row = c.execute("SELECT value FROM app_settings WHERE user_id=? AND key=?", (user_id, "billing_biz_reg_number")).fetchone()
    return row["value"] if row else None
...
CONFIG = {
    ...
    "work_logs": ({"content", "log_date", "task_id", "tags", "duration_minutes", "link_url", "links", "color", "log_time", "billable", "checklist", "priority", "estimated_minutes", "custom_fields", "invoiced_at", "hourly_rate_override", "client_name", "reminder_minutes_before"}, None),
}
...
TODO_SERIES_EDITABLE_FIELDS = {"title", "memo", "tags", "link_url", "color", "priority", "todo_time", "estimated_minutes"}
...
EVENT_SERIES_EDITABLE_FIELDS = {"title", "description", "location", "tags", "link_url", "color", "priority", "estimated_minutes"}
...
def audit_log_list(limit: int = 100, offset: int = Query(0, ge=0), start: str = None, end: str = None, user=Depends(require_user)):
    clauses, args = [], []
    if start:
    ...
    items = rows("audit_logs", user, f"{where}ORDER BY created_at DESC, id DESC LIMIT ? OFFSET ?", (*args, max(1, min(limit, 500)), offset))
    for item in items:
...
def public_task(token: str, request: Request, password: str | None = None):
    ...
    with connection() as c:
        item = c.execute("""SELECT title,description,status,priority,progress,start_date,due_date,
          tags,link_url,checklist,custom_fields,created_at,updated_at,public_token_password_hash FROM tasks
...
def public_event(token: str, request: Request, password: str | None = None):
    ...
    with connection() as c:
        item = c.execute("""SELECT title,description,location,start_at,end_at,tags,link_url,checklist,custom_fields,created_at,updated_at,public_token_password_hash FROM events
          WHERE public_token=? AND deleted_at IS NULL
          AND (public_token_expires_at IS NULL OR public_token_expires_at > ?)""", (token, now())).fetchone()
    if not item:
        raise HTTPException(404, "공유 링크를 찾을 수 없습니다.")
...
def public_todo(token: str, request: Request, password: str | None = None):
    ...
    with connection() as c:
        item = c.execute("""SELECT title,memo,priority,completed,todo_date,todo_time,tags,link_url,checklist,custom_fields,created_at,public_token_password_hash FROM todos
          WHERE public_token=? AND deleted_at IS NULL
          AND (public_token_expires_at IS NULL OR public_token_expires_at > ?)""", (token, now())).fetchone()
    if not item:
        raise HTTPException(404, "공유 링크를 찾을 수 없습니다.")
...
def public_work_log(token: str, request: Request, password: str | None = None):
    ...
    with connection() as c:
        item = c.execute("""SELECT content,log_date,log_time,duration_minutes,billable,priority,tags,link_url,checklist,custom_fields,created_at,public_token_password_hash FROM work_logs
          WHERE public_token=? AND deleted_at IS NULL
          AND (public_token_expires_at IS NULL OR public_token_expires_at > ?)""", (token, now())).fetchone()
    if not item:
        raise HTTPException(404, "공유 링크를 찾을 수 없습니다.")
...
def workflow_settings_update(payload: dict = Body(...), user=Depends(require_user)):
    ...
    if "billing_biz_reg_number" in value:
        biz_reg_number = (value["billing_biz_reg_number"] or "").strip() or None
        with connection() as c:
            if biz_reg_number is None:
                c.execute("DELETE FROM app_settings WHERE user_id=? AND key=?", (user, "billing_biz_reg_number"))
            else:
                c.execute("""INSERT INTO app_settings(user_id,key,value,updated_at) VALUES(?,?,?,?)
                  ON CONFLICT(user_id,key) DO UPDATE SET value=excluded.value,updated_at=excluded.updated_at""",
