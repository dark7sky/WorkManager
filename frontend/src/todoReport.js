...
export const todosToPrintableReport = (todos, { generatedAt = new Date().toISOString(), title = 'WorkManager Todo 보고서' } = {}) => {
  ...
  const rows = todos.map(todo => `<tr>
      ...
      <td>${escapeHtml(estimateSummary(todo.estimated_minutes))}</td>
      <td>${linkCell(todo.link_url)}</td>
    </tr>`).join('')
...
<body>
  ...
  <table>
    <thead><tr><th>Todo</th><th>상태</th><th>우선순위</th><th>시간</th><th>태그</th><th>체크리스트</th><th>예상 소요시간</th><th>링크</th></tr></thead>
    <tbody>${rows || '<tr><td colspan="8">표시할 Todo가 없습니다.</td></tr>'}</tbody>
  </table>
</body>
</html>`
}
