...
export default function Settings({ theme, setTheme, teamMembers = [], tasks = [], teamMemberCapacities = {}, setTeamMemberCapacity, setTeamMembers, notify, onDataChanged, canInstall, onInstall, notificationPermission, onEnableNotifications }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setAiDraft({
        provider,
        api_key: '',
        base_url: ai?.source === 'user' ? (ai?.base_url || defaults.base_url) : defaults.base_url,
        model: ai?.source === 'user' ? (ai?.model || defaults.model) : defaults.model,
      })
  ...
  const saveAi = async event => {
    ...
    } else if (!(aiConfig?.source === 'user' && aiConfig?.api_key_set)) {
    ...
    try {
      ...
      setAiConfig(next)
      setAiDraft(current => ({ ...current, api_key: '' }))
  ...
  return <>
    ...
    <div className="content settings-page">
      ...
      <section className="settings-card">
        <div className="settings-heading"><span><Bot /></span><div><h2>AI 서비스</h2><p>각 계정이 쓸 AI 공급자와 키를 따로 저장합니다.</p></div><em className={`status-pill ${aiConfig?.configured ? 'online' : ''}`}>{aiConfig?.configured ? '설정됨' : '미설정'}</em></div>
        {!aiConfig ? <div className="skeleton lines" /> : <form className="ai-settings-form" onSubmit={saveAi}>
          <div className="ai-settings-grid">
            <label>공급자<select value={aiDraft.provider} onChange={event => { const provider = event.target.value; const defaults = aiDefaults[provider] || aiDefaults.openai; setAiDraft(current => ({ ...current, provider, base_url: defaults.base_url, model: defaults.model })) }}><option value="openai">OpenAI</option><option value="gemini">Google Gemini</option></select></label>
            <label>API key<input type="password" value={aiDraft.api_key} onChange={event => setAiDraft(current => ({ ...current, api_key: event.target.value }))} placeholder={aiConfig.api_key_set ? '기존 키 유지' : '필수'} /></label>
            <label>모델<input value={aiDraft.model} onChange={event => setAiDraft(current => ({ ...current, model: event.target.value }))} placeholder={aiDefault.model} /></label>
          ...
          <div className="integration-body ai-settings-footer"><div><strong>{aiConfig.provider_name || (aiProvider === 'gemini' ? 'Google Gemini' : 'OpenAI')}</strong><small>설정은 현재 Google 계정에만 저장됩니다. 키는 암호화해 보관합니다.</small></div><button className="primary" disabled={busy === 'ai-save'}>{busy === 'ai-save' ? <LoaderCircle className="spin" /> : <Bot />} 저장</button></div>
        </form>}
        {aiConfig ? <dl className="diagnostics"><div><dt>엔진</dt><dd>{aiConfig.provider_name || aiConfig.provider}</dd></div><div><dt>모델</dt><dd>{aiConfig.model || 'API 미설정'}</dd></div><div><dt>상태</dt><dd>{aiConfig.message || '사용 가능'}</dd></div><div><dt>저장 위치</dt><dd>{aiConfig.source_label || aiConfig.source || '서버 기본값'}</dd></div></dl> : null}
      </section>
