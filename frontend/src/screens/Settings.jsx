  ...
  return <>
    ...
    <div className="content settings-page">
      ...
      <section className="settings-card">
        ...
        <label>일정 시작 전 알림 시점<select value={eventAlertLead} onChange={e => { const minutes = Number(e.target.value); setEventAlertLead(minutes); localStorage.setItem(EVENT_ALERT_LEAD_STORAGE_KEY, String(minutes)) }}>{EVENT_ALERT_LEAD_OPTIONS.map(minutes => <option key={minutes} value={minutes}>{minutes===0?'끄기':`${minutes}분 전`}</option>)}</select></label>
        <label>할 일 예정 시간 전 알림 시점<select value={todoAlertLead} onChange={e => { const minutes = Number(e.target.value); setTodoAlertLead(minutes); localStorage.setItem(TODO_ALERT_LEAD_STORAGE_KEY, String(minutes)) }}>{TODO_ALERT_LEAD_OPTIONS.map(minutes => <option key={minutes} value={minutes}>{minutes===0?'끄기':`${minutes}분 전`}</option>)}</select></label>
        <label>업무 마감 시간 전 알림 시점<select value={taskAlertLead} onChange={e => { const minutes = Number(e.target.value); setTaskAlertLead(minutes); localStorage.setItem(TASK_ALERT_LEAD_STORAGE_KEY, String(minutes)) }}>{TASK_ALERT_LEAD_OPTIONS.map(minutes => <option key={minutes} value={minutes}>{minutes===0?'끄기':`${minutes}분 전`}</option>)}</select></label>
        <label>업무 기록 예정 시간 전 알림 시점<select value={logAlertLead} onChange={e => { const minutes = Number(e.target.value); setLogAlertLead(minutes); localStorage.setItem(LOG_ALERT_LEAD_STORAGE_KEY, String(minutes)) }}>{LOG_ALERT_LEAD_OPTIONS.map(minutes => <option key={minutes} value={minutes}>{minutes===0?'끄기':`${minutes}분 전`}</option>)}</select></label>
      ...
      <section className="settings-card">
        ...
        {!sessions ? <div className="skeleton lines" /> : sessions.length ? <ul className="error-log-list session-list">{sessions.map(s => <li key={s.id}><time>{new Intl.DateTimeFormat('ko-KR', { dateStyle: 'medium', timeStyle: 'short', hour12: false }).format(new Date(s.last_seen_at * 1000))}</time><div><strong>{s.current ? '현재 세션' : '다른 세션'}{s.device ? ` · ${s.device}` : ''}</strong><p>생성: {new Intl.DateTimeFormat('ko-KR', { dateStyle: 'medium', timeStyle: 'short', hour12: false }).format(new Date(s.created_at * 1000))} · 만료: {new Intl.DateTimeFormat('ko-KR', { dateStyle: 'medium', timeStyle: 'short', hour12: false }).format(new Date(s.expires_at * 1000))}</p></div>{!s.current ? <button className="secondary" disabled={busy === `session-${s.id}`} onClick={() => revokeSession(s.id)}>로그아웃</button> : null}</li>)}</ul> : <p className="empty-state">활성 세션이 없습니다.</p>}
      </section>
      <section className="settings-card">
