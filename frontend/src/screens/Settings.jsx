...
import { addTeamMember, removeTeamMember } from '../teamMembers'
import { summarizeTeamMemberRoster } from '../taskFilters'

const themes = [['auto',Monitor,'시스템'],['light',Sun,'라이트'],['dark',Moon,'다크']]
export default function Settings({ theme, setTheme, teamMembers = [], tasks = [], setTeamMembers, notify, onDataChanged, canInstall, onInstall, notificationPermission, onEnableNotifications }) {
  ...
  const memberRoster = useMemo(() => summarizeTeamMemberRoster(tasks, teamMembers, todayIso), [tasks, teamMembers, todayIso])
  const load = async () => {
    setError('')
    try {
  ...
  const deleteMember = name => { setTeamMembers?.(removeTeamMember(teamMembers, name)); notify('팀원을 제거했습니다.') }
