...
export default function Performance({ notify, onDataChanged }) {
  ...
  const printInvoice = useCallback((clientFilter) => {
    if (!data || invalidRange) return
    const html = workLogsToPrintableInvoice(data.work_logs || [], { start: dates[0], end: dates[1], hourlyRate: data.summary?.billing_hourly_rate, clientName: clientFilter || data.summary?.billing_client_name, filterClientName: clientFilter, bizRegNumber: data.summary?.billing_biz_reg_number, vatIncluded: data.summary?.billing_vat_included, generatedAt: new Date().toISOString() })
    const win = window.open('', '_blank')
    if (!win) return
    win.document.open(); win.document.write(html); win.document.close()
  ...
  const markInvoiced = useCallback(async (clientFilter) => {
    const pending = billableWorkLogs(data?.work_logs || [], clientFilter)
  ...
  const unmarkInvoiced = useCallback(async (clientFilter) => {
    const invoiced = invoicedWorkLogs(data?.work_logs || [], clientFilter)
  ...
  return <><Header title="성과" subtitle="기간별 업무 기록을 모아보고, 평가 자료와 다음 행동으로 연결하세요."/><div className="content performance-page">
    ...
    {reportLoading && !data ? <div className="ai-empty"><LoaderCircle className="spin"/> 기록을 모으는 중입니다.</div> : <>
      ...
      {clientBreakdown.length ? <section className="performance-timeline tag-breakdown"><h2>고객별 소요 시간 <small>{clientBreakdown.length}개 고객</small></h2>{clientBreakdown.map(c => { const clientPending = billableWorkLogs(data?.work_logs || [], c.client_name), clientInvoiced = invoicedWorkLogs(data?.work_logs || [], c.client_name); return <div className="tag-breakdown-row" key={c.client_name}><span className="tag-breakdown-name">{c.client_name}</span><span className="tag-breakdown-bar"><i style={{width: `${Math.round(c.tracked_minutes / maxClientMinutes * 100)}%`}}/></span><span className="tag-breakdown-figures">{formatDuration(c.tracked_minutes)}{c.billable_amount ? ` · 청구 ${Math.round(c.billable_amount).toLocaleString('ko-KR')}원` : ''}</span>{clientPending.length ? <button type="button" className="link-button" disabled={reportLoading || invalidRange} onClick={() => printInvoice(c.client_name)} title={`${c.client_name} 청구서 PDF`}><Wallet size={14}/></button> : null}{clientPending.length ? <button type="button" className="link-button" disabled={reportLoading || invalidRange} onClick={() => markInvoiced(c.client_name)} title={`${c.client_name} 청구 완료 표시`}><CheckCircle2 size={14}/></button> : null}{clientInvoiced.length ? <button type="button" className="link-button" disabled={reportLoading || invalidRange} onClick={() => unmarkInvoiced(c.client_name)} title={`${c.client_name} 청구 완료 취소`}><RotateCcw size={14}/></button> : null}</div> })}</section> : null}
