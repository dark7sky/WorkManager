...
import { AlertTriangle, CheckSquare, ChevronLeft, ChevronRight, Copy, Download, ExternalLink, FileText, Flag, MapPin, Paperclip, Plus, Search, Star, Upload } from 'lucide-react'
...
export default function Calendar({ events, tasks = [], onOpenTask, onCreate, onUpdate, onDelete, onBulkDelete, onDataChanged, notify }) {
  ...
  return <>
    ...
    {(editing || newDate) ? <Modal title={editing ? '일정 수정' : '새 일정'} onClose={close}>{editing?.sync_state === 'conflict' ? <ConflictPanel event={editing} notify={notify} onResolved={async () => { await onDataChanged(); close() }}/> : null}<EventForm event={editing} date={newDate || dateKey(cursor)} allEvents={events} onCancel={close} onDelete={() => setDeleting(true)} onDuplicate={async e => { const ok = await onCreate(buildEventDuplicatePayload(e)); if (ok) close() }} onSave={async (data, applyToSeries) => { const ok = await (editing ? onUpdate(editing, data, applyToSeries) : onCreate(data)); if (ok) close(); return ok }}/></Modal> : null}
