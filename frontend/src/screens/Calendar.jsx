...
import { AlertTriangle, ChevronLeft, ChevronRight, Copy, Download, ExternalLink, MapPin, Plus, Search, Upload } from 'lucide-react'
...
function EventForm({ event, date, onSave, onDelete, onDuplicate, onCancel }) {
  ...
  const submit = async formEvent => {
    ...
    const payload = { ...data, title: data.title.trim(), description: data.description.trim(), location: data.location.trim(), start_at: `${data.start_at}:00`, end_at: `${data.end_at}:00`, tags, link_url: linkUrl && /^https?:\/\//.test(linkUrl) ? linkUrl : null }
