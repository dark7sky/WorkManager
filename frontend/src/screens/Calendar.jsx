...
export default function Calendar({ events, tasks = [], onOpenTask, onCreate, onUpdate, onDelete, onBulkDelete, onBulkAddTag, onBulkPostpone, onBulkPriority, onDataChanged, onViewHistory, notify }) {
  ...
  return <>
    ...
    {(editing || newDate) ? <Modal title={editing ? '일정 수정' : '새 일정'} onClose={close}>{editing?.sync_state === 'conflict' ? <ConflictPanel event={editing} notify={notify} onResolved={async () => { await onDataChanged(); close() }}/> : null}<EventForm event={editing} date={newDate || dateKey(cursor)} allEvents={events} onCancel={close} onDelete={() => setDeleting(true)} onDuplicate={async e => { const ok = await onCreate(buildEventDuplicatePayload(e)); if (ok) close() }} onViewHistory={onViewHistory} onSave={async (data, applyToSeries) => { const ok = await (editing ? onUpdate(editing, data, applyToSeries) : onCreate(data)); if (ok) close(); return ok }}/></Modal> : null}
