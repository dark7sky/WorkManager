...
function EventForm({ event, date, allEvents = [], onSave, onDelete, onDuplicate, onPostpone, onArchive, onPromote, onViewHistory, onCopyLink, onCancel }) {
...
export default function Calendar({ events, tasks = [], loading, onOpenTask, onCreate, onUpdate, onDelete, onBulkDelete, onBulkAddTag, onBulkPostpone, onBulkPriority, onBulkColor, onBulkDuplicate, onBulkArchive, onBulkPromote, onArchive, onPromoteEvent, onDataChanged, onViewHistory, focusTag, focusEventId, notify }) {
  useEffect(() => { if (!focusEventId) return; const target = events.find(e => e.id === focusEventId); if (target) setEditing(target) }, [focusEventId, events])
  ...
  const exportCsv = () => {
    const { headers, rows } = filterCsvColumns(eventHeaders, eventRows(sorted, pinnedIds), csvColumns)
  ...
  return <>
    ...
    <div className={`content cal-view-${view}`}>
      <div className="calendar-tools"><div className="month-switch"><button className="secondary" onClick={() => setCursor(new Date())}>오늘</button><button className="icon-button" aria-label={view === 'day' ? '이전 날' : view === 'week' ? '이전 주' : view === 'year' ? '이전 해' : '이전 달'} onClick={() => setCursor(date => view === 'day' ? new Date(date.getFullYear(), date.getMonth(), date.getDate() - 1) : view === 'week' ? new Date(date.getFullYear(), date.getMonth(), date.getDate() - 7) : view === 'year' ? new Date(date.getFullYear() - 1, date.getMonth(), 1) : new Date(date.getFullYear(), date.getMonth() - 1, 1))}><ChevronLeft/></button><h2>{view === 'year' ? `${cursor.getFullYear()}년` : view === 'month' ? `${cursor.getFullYear()}년 ${cursor.getMonth() + 1}월` : view === 'day' ? `${cursor.getFullYear()}년 ${cursor.getMonth() + 1}월 ${cursor.getDate()}일` : (() => { const weekStart = new Date(cursor.getFullYear(), cursor.getMonth(), cursor.getDate() - cursor.getDay()), weekEnd = new Date(weekStart.getFullYear(), weekStart.getMonth(), weekStart.getDate() + 6); return `${weekStart.getFullYear()}년 ${weekStart.getMonth() + 1}월 ${weekStart.getDate()}일 – ${weekEnd.getMonth() + 1}월 ${weekEnd.getDate()}일` })()}</h2>{loading ? <span className="status-pill">동기화 중…</span> : null}<button className="icon-button" aria-label={view === 'day' ? '다음 날' : view === 'week' ? '다음 주' : view === 'year' ? '다음 해' : '다음 달'} onClick={() => setCursor(date => view === 'day' ? new Date(date.getFullYear(), date.getMonth(), date.getDate() + 1) : view === 'week' ? new Date(date.getFullYear(), date.getMonth(), date.getDate() + 7) : view === 'year' ? new Date(date.getFullYear() + 1, date.getMonth(), 1) : new Date(date.getFullYear(), date.getMonth() + 1, 1))}><ChevronRight/></button></div><div className="view-switch"><button className={view === 'day' ? 'active' : ''} onClick={() => setView('day')}>일</button><button className={view === 'month' ? 'active' : ''} onClick={() => setView('month')}>월</button><button className={view === 'week' ? 'active' : ''} onClick={() => setView('week')}>주</button><button className={view === 'year' ? 'active' : ''} onClick={() => setView('year')}>년</button></div><button className="secondary" onClick={printReport} disabled={!sorted.length}><FileText size={17}/>PDF</button><button className="secondary" onClick={exportIcs} disabled={!sorted.length}><Download size={17}/>ICS</button><div className="badge-visibility-menu"><button type="button" className="secondary" onClick={() => setCsvColumnMenuOpen(o => !o)} aria-expanded={csvColumnMenuOpen} aria-label="내보낼 열 설정"><SlidersHorizontal size={17}/>내보낼 열</button>{csvColumnMenuOpen ? <div className="badge-visibility-dropdown" role="menu">{EVENT_CSV_COLUMN_OPTIONS.map(opt => <label key={opt.index}><input type="checkbox" checked={csvColumns.has(opt.index)} onChange={() => toggleCsvColumn(opt.index)}/>{opt.label}</label>)}</div> : null}</div><button className="secondary" onClick={exportCsv} disabled={!sorted.length}><Download size={17}/>CSV</button><button className="secondary" onClick={exportExcel} disabled={!sorted.length}><Download size={17}/>Excel</button><button className="secondary" onClick={() => importInputRef.current?.click()}><Upload size={17}/>ICS 가져오기</button><input ref={importInputRef} type="file" accept=".ics,text/calendar" hidden onChange={importIcs}/><button className="secondary" onClick={() => csvImportInputRef.current?.click()}><Upload size={17}/>CSV 가져오기</button><input ref={csvImportInputRef} type="file" accept=".csv,text/csv" hidden onChange={importCsv}/></div>
