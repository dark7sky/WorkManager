...
import { Download, Search, SlidersHorizontal, UserRound } from 'lucide-react'
...
export default function Tasks({tasks,onNew,onEdit,onProgress}){
  const [query,setQuery]=useState(''),[status,setStatus]=useState('active'),[selectedTags,setSelectedTags]=useState([]),[draft,setDraft]=useState({})
  ...
  const shown=tasks.filter(t=>{
    const q=query.trim().toLowerCase(), matches=!q||`${t.title} ${t.description||''} ${t.assignee_name||''} ${(t.tags||[]).join(' ')}`.toLowerCase().includes(q)
    const matchesStatus=status==='all'||(status==='overdue'?isOverdue(t):status==='active'?t.status!=='done':status==='in_progress'?['in_progress','doing'].includes(t.status):t.status===status)
    const matchesTags=!selectedTags.length||selectedTags.every(tag=>(t.tags||[]).includes(tag))
    return matches&&matchesStatus&&matchesTags
  })
  const exportShown=()=>{
  ...
  return <><Header title="업무 관리" subtitle="업무 일정과 진행률을 한눈에 관리하세요." action="새 업무" onAction={onNew}/><div className="content">
    <div className="toolbar task-toolbar"><div className="search"><Search/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="제목, 메모, 태그 검색" /></div><div className="task-toolbar-actions"><label className="filter-select"><SlidersHorizontal/><span>상태</span><select value={status} onChange={e=>setStatus(e.target.value)}><option value="active">진행 업무</option><option value="overdue">지연 업무</option><option value="all">전체</option><option value="todo">할 일</option><option value="in_progress">진행 중</option><option value="done">완료</option></select></label><button className="secondary" onClick={exportShown} disabled={!shown.length}><Download size={17}/>CSV</button></div></div><TagFilter tags={allTags} selected={selectedTags} onChange={setSelectedTags}/>
