...
import { BellRing, CalendarRange, CheckCircle2, Download, FileText, Flag, GitPullRequestArrow, RotateCcw, Search, ShieldCheck, SlidersHorizontal, UserRound, UsersRound, UserX } from 'lucide-react'
...
export default function Tasks({tasks,onNew,onEdit,onProgress,onApprove}){
  ...
  return <><Header title="업무 관리" subtitle="업무 일정과 진행률을 한눈에 관리하세요." action="새 업무" onAction={onNew}/><div className="content">
    ...
    {pendingApprovals.length?<section className="approval-alert" aria-label="완료 업무 승인 대기"><ShieldCheck aria-hidden="true"/><div><strong>승인 대기</strong><p>완료된 업무 {pendingApprovals.length}개가 검토를 기다리고 있습니다.</p></div><button type="button" className="secondary" onClick={()=>setStatus('done')}>완료 보기</button></section>:null}
