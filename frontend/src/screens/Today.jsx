...
export default function Today(props) {
  ...
  const submitLog = async event => {
    ...
    setSaving('log')
    if (await onAddLog(logDraft.trim(), logTags, logTaskId ? Number(logTaskId) : null, logMinutes ? Number(logMinutes) : null, logLink.trim(), normalizedLinks(logLinks), logColor)) {
      setLogDraft('')
      setLogTags([])
      setLogTaskId('')
      setLogMinutes('')
  ...
  const beginEdit = (type, item) => {
    ...
    if (type === 'log') { setEditTaskId(item.task_id ?? ''); setEditMinutes(item.duration_minutes ?? ''); setEditLogLink(item.link_url || ''); setEditLogLinks(item.links || []); setEditLogColor(item.color || '') }
    if (type === 'todo') { setEditRecurrence(item.recurrence_rule || ''); setEditRecurrenceEnd(item.recurrence_end_date || ''); setEditPriority(item.priority || 'normal'); setEditLink(item.link_url || ''); setEditTodoLinks(item.links || []); setEditMemo(item.memo || ''); setEditColor(item.color || '') }
  }
  const saveEdit = async item => {
    ...
    const ok = edit.type === 'todo'
      ? await onUpdateTodo(item.id, editText.trim(), editTags, editRecurrence, editPriority, editLink.trim(), editRecurrenceEnd, editMemo.trim(), editColor, normalizedLinks(editTodoLinks))
      : await onUpdateLog(item.id, editText.trim(), editTags, editTaskId ? Number(editTaskId) : null, editMinutes ? Number(editMinutes) : null, editLogLink.trim(), normalizedLinks(editLogLinks), editLogColor)
    if (ok) setEdit(null)
    setSaving('')
  }
