...
import { ArrowUpRight, CalendarClock, Check, CheckCircle2, ChevronRight, Circle, Clock3, Copy, Download, ExternalLink, Pencil, Play, Plus, Sparkles, Square, Star, Tag, Trash2, Upload, X } from 'lucide-react'
...
export default function Today(props) {
  ...
  const submitLog = async event => {
    ...
    if (!logDraft.trim()) return
    setSaving('log')
    if (await onAddLog(logDraft.trim(), logTags, logTaskId ? Number(logTaskId) : null, logMinutes ? Number(logMinutes) : null, logLink.trim(), normalizedLinks(logLinks), logColor, logTime || null)) {
      setLogDraft('')
      setLogTags([])
      setLogTaskId('')
      setLogMinutes('')
  ...
  const beginEdit = (type, item) => {
    ...
    if (type === 'log') { setEditTaskId(item.task_id ?? ''); setEditMinutes(item.duration_minutes ?? ''); setEditLogLink(item.link_url || ''); setEditLogLinks(item.links || []); setEditLogColor(item.color || ''); setEditLogTime(item.log_time || '') }
  ...
  const saveEdit = async item => {
    ...
    const ok = edit.type === 'todo'
      ...
      : await onUpdateLog(item.id, editText.trim(), editTags, editTaskId ? Number(editTaskId) : null, editMinutes ? Number(editMinutes) : null, editLogLink.trim(), normalizedLinks(editLogLinks), editLogColor, editLogTime || null)
    if (ok) setEdit(null)
    setSaving('')
  }
