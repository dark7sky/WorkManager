...
export default function Today(props) {
  ...
  const beginEdit = (type, item) => {
    ...
    if (type === 'log') { setEditTaskId(item.task_id ?? ''); setEditMinutes(item.duration_minutes ?? ''); setEditLogLink(item.link_url || ''); setEditLogLinks(item.links || []); setEditLogColor(item.color || ''); setEditLogTime(item.log_time || ''); setEditLogBillable(!!item.billable); setEditLogChecklist(item.checklist || []) }
  ...
  const saveEdit = async item => {
    ...
    const ok = edit.type === 'todo'
      ? await onUpdateTodo(item.id, editText.trim(), editTags, editRecurrence, editPriority, editLink.trim(), editRecurrenceEnd, editMemo.trim(), editColor, normalizedLinks(editTodoLinks), editTodoTime, normalizedChecklist(editTodoChecklist))
      : await onUpdateLog(item.id, editText.trim(), editTags, editTaskId ? Number(editTaskId) : null, editMinutes ? Number(editMinutes) : null, editLogLink.trim(), normalizedLinks(editLogLinks), editLogColor, editLogTime || null, editLogBillable, normalizedChecklist(editLogChecklist))
    if (ok) setEdit(null)
    setSaving('')
  }
