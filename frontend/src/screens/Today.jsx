...
export default function Today(props) {
  const {
    ...
    onBulkCompleteTodo, onBulkDeleteTodo, onBulkAddTagTodo, onBulkPostponeTodo, onBulkPriorityTodo, onBulkColorTodo, onBulkDuplicateTodo, onBulkArchiveTodo, onBulkPromoteTodo,
    ...
    onBulkDeleteLog, onBulkAddTagLog, onBulkPostponeLog, onBulkPriorityLog, onBulkColorLog, onBulkBillableLog, onBulkDuplicateLog, onBulkArchiveLog, onBulkPromoteLog, focusTag, focusTodoId, focusLogId, notify, onOpenTask,
  } = props
  ...
  const createEditTodoShareLink = async todo => {
    ...
    try { const res = await api.shareTodo(todo.id, editTodoShareExpiryDays ? Number(editTodoShareExpiryDays) : undefined); setEditTodoShareToken(res.public_token); setEditTodoShareExpiresAt(res.public_token_expires_at || '') }
    catch (e) { notify?.(e.message, 'error') }
    finally { setEditTodoShareBusy(false) }
  }
  const revokeEditTodoShareLink = async todo => {
    setEditTodoShareBusy(true)
    try { await api.unshareTodo(todo.id); setEditTodoShareToken(''); setEditTodoShareExpiresAt('') }
    catch (e) { notify?.(e.message, 'error') }
    finally { setEditTodoShareBusy(false) }
  }
  ...
  const createEditLogShareLink = async log => {
    ...
    try { const res = await api.shareLog(log.id, editLogShareExpiryDays ? Number(editLogShareExpiryDays) : undefined); setEditLogShareToken(res.public_token); setEditLogShareExpiresAt(res.public_token_expires_at || '') }
    catch (e) { notify?.(e.message, 'error') }
    finally { setEditLogShareBusy(false) }
  }
  const revokeEditLogShareLink = async log => {
    setEditLogShareBusy(true)
    try { await api.unshareLog(log.id); setEditLogShareToken(''); setEditLogShareExpiresAt('') }
    catch (e) { notify?.(e.message, 'error') }
    finally { setEditLogShareBusy(false) }
  }
  ...
  const submitLog = async event => {
    ...
    if (Object.keys(errors).length) return
    setSaving('log')
    if (await onAddLog(logDraft.trim(), logTags, logTaskId ? Number(logTaskId) : null, logMinutes ? Number(logMinutes) : null, logLink.trim(), normalizedLinks(logLinks), logColor, logTime || null, logBillable, logPriority, logEstimate ? Number(logEstimate) : null, normalizedCustomFields(logCustomFields))) {
      setLogDraft('')
      setLogFieldErrors({})
      setLogTags([])
      setLogTaskId('')
