...
import { ArrowUpRight, CalendarClock, Check, CheckCircle2, ChevronRight, Circle, Clock3, Copy, Download, ExternalLink, FileText, Flag, History, Paperclip, Pencil, Play, Plus, SkipForward, Sparkles, Square, Star, Tag, Trash2, Upload, X } from 'lucide-react'
...
export default function Today(props) {
  const {
    ...
    onAddTodo, onUpdateTodo, onToggleTodo, onDeleteTodo, onDuplicateTodo, onPromoteTodo, onSkipTodoRecurrence, onClearCompletedTodos, onCarryOverTodos, onImportTodos, onPostponeTodo,
    onBulkCompleteTodo, onBulkDeleteTodo, onBulkAddTagTodo, onBulkPostponeTodo, onBulkPriorityTodo, onBulkDuplicateTodo,
    onAddLog, onUpdateLog, onDeleteLog, onDuplicateLog, onPromoteLog, onPostponeLog, onImportLogs, onToggleTask, goAI, onViewHistoryTodo, onViewHistoryLog,
    onBulkDeleteLog, onBulkAddTagLog, onBulkPostponeLog, onBulkPriorityLog, onBulkDuplicateLog, notify,
  } = props
  ...
  const submitTodo = async event => {
    ...
    const errors = validateTodoForm({ title: todoDraft })
  ...
  const submitLog = async event => {
    ...
    if (Object.keys(errors).length) return
    setSaving('log')
    if (await onAddLog(logDraft.trim(), logTags, logTaskId ? Number(logTaskId) : null, logMinutes ? Number(logMinutes) : null, logLink.trim(), normalizedLinks(logLinks), logColor, logTime || null, logBillable, logPriority)) {
      setLogDraft('')
      setLogFieldErrors({})
      setLogTags([])
      setLogTaskId('')
  ...
  const beginEdit = (type, item) => {
    ...
    if (type === 'log') { setEditTaskId(item.task_id ?? ''); setEditMinutes(item.duration_minutes ?? ''); setEditLogLink(item.link_url || ''); setEditLogLinks(item.links || []); setEditLogColor(item.color || ''); setEditLogTime(item.log_time || ''); setEditLogBillable(!!item.billable); setEditLogPriority(item.priority || 'normal'); setEditLogChecklist(item.checklist || []) }
  ...
  const saveEdit = async item => {
    ...
    const ok = edit.type === 'todo'
      ...
      : await onUpdateLog(item.id, editText.trim(), editTags, editTaskId ? Number(editTaskId) : null, editMinutes ? Number(editMinutes) : null, editLogLink.trim(), normalizedLinks(editLogLinks), editLogColor, editLogTime || null, editLogBillable, normalizedChecklist(editLogChecklist), editLogPriority)
    if (ok) setEdit(null)
    setSaving('')
  }
    ...
    <div className="content today-grid">
      <section className="focus-panel">
        ...
        {shownTodos.length ? <div className="todo-list">{shownTodos.map(todo => <div className={`todo-row ${todo.completed ? 'completed' : ''} ${todo.priority === 'high' ? 'priority-high' : ''} ${selectedTodoIds.has(todo.id) ? 'row-selected' : ''}`} style={eventColorHex(todo.color) ? { borderLeft: `3px solid ${eventColorHex(todo.color)}` } : undefined} key={todo.id}>
          ...
          <span className="row-actions">{editable('todo', todo) ? <><button aria-label="수정 취소" onClick={() => setEdit(null)}><X/></button><button aria-label="수정 저장" disabled={saving === `todo-${todo.id}`} onClick={() => saveEdit(todo)}><Check/></button></> : <><button aria-label={`${todo.title} 수정`} onClick={() => beginEdit('todo', todo)}><Pencil/></button><button className={`task-pin${pinnedTodoIds.has(todo.id) ? ' pinned' : ''}`} aria-label={`${todo.title} ${pinnedTodoIds.has(todo.id) ? '고정 해제' : '고정'}`} title={pinnedTodoIds.has(todo.id) ? '고정 해제' : '목록 상단 고정'} onClick={() => togglePin(todo)}><Star/></button><button aria-label={`${todo.title} 복제`} onClick={() => onDuplicateTodo(todo)}><Copy/></button>{onPostponeTodo && !todo.completed ? <button aria-label={`${todo.title} 하루 미루기`} title="하루 미루기" onClick={() => onPostponeTodo(todo)}><CalendarClock/></button> : null}<button aria-label={`${todo.title} 업무로 전환`} title="업무로 전환" onClick={() => onPromoteTodo(todo)}><ArrowUpRight/></button>{onViewHistoryTodo ? <button aria-label={`${todo.title} 변경 이력 보기`} title="변경 이력 보기" onClick={() => onViewHistoryTodo(todo)}><History/></button> : null}{todo.recurrence_rule && !todo.completed ? <button aria-label={`${todo.title} 다음 회차로 건너뛰기`} title="다음 회차로 건너뛰기" onClick={() => onSkipTodoRecurrence(todo)}><SkipForward/></button> : null}</>}<button className="danger-icon" aria-label={`${todo.title} 삭제`} onClick={() => onDeleteTodo(todo)}><Trash2/></button></span>
