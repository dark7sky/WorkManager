...
test('buildTodoDuplicatePayload copies fields, resets completion and dates to today', () => {
  const todo = { id: 3, title: '우유 사기', completed: true, tags: ['생활'], recurrence_rule: 'daily', recurrence_end_date: '2026-08-01', todo_date: '2026-07-01', priority: 'high' }
  ...
  assert.equal(result.recurrence_end_date, '2026-08-01')
  assert.equal(result.priority, 'high')
})

test('buildTodoDuplicatePayload drops a stray recurrence_end_date when there is no recurrence_rule', () => {
  const result = buildTodoDuplicatePayload({ title: '메모', recurrence_end_date: '2026-08-01' })
  assert.equal(result.recurrence_rule, null)
  assert.equal(result.recurrence_end_date, null)
})
