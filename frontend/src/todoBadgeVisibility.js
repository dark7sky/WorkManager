const STORAGE_KEY = 'wm-todo-badge-visibility'

export const TODO_BADGE_OPTIONS = [
...
export const loadTodoBadgeVisibility = (storage = localStorage) => {
  try {
...
export const saveTodoBadgeVisibility = (visibleKeys, storage = localStorage) => {
  const value = Object.fromEntries(ALL_KEYS.map(k => [k, visibleKeys.has(k)]))
  storage.setItem(STORAGE_KEY, JSON.stringify(value))
}

export const toggleTodoBadgeVisibility = (visibleKeys, key) => {
