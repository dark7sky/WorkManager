...
export default function TrashSection({ notify, onDataChanged }) {
  ...
  const purgeOne = async () => {
    ...
    setBusy(`${item.table}-${item.id}`)
    try {
      await api.purgeTrashItem(item.table, item.id)
      setItems(current => current.filter(value => value !== item))
      setSelected(current => { const next = new Set(current); next.delete(`${item.table}-${item.id}`); return next })
      notify('항목을 영구 삭제했습니다.')
    } catch (error) { notify(error.message, 'error') }
    finally { setBusy(''); setPurgeTarget(null) }
  }
  const purgeSelected = async () => {
    setBusy('bulk')
    const targets = shown.filter(item => selected.has(`${item.table}-${item.id}`))
    try {
      await Promise.all(targets.map(item => api.purgeTrashItem(item.table, item.id)))
      setItems(current => current.filter(item => !selected.has(`${item.table}-${item.id}`)))
      notify(`${targets.length}개 항목을 영구 삭제했습니다.`)
      clearSelected()
    } catch (error) { notify(error.message, 'error') }
    finally { setBusy(''); setBulkPurge(false) }
  }
  const purge = async () => {
    setBusy('cleanup')
    try {
  ...
  return <section className="settings-card">
    ...
    {purgeTarget ? <ConfirmDialog title="항목을 영구 삭제할까요?" message="이 항목을 즉시 영구 삭제합니다. 이 작업은 되돌릴 수 없습니다." confirmLabel="영구 삭제" busy={busy === `${purgeTarget.table}-${purgeTarget.id}`} onClose={() => setPurgeTarget(null)} onConfirm={purgeOne}/> : null}
    {bulkPurge ? <ConfirmDialog title={`선택한 ${selected.size}개 항목을 영구 삭제할까요?`} message="선택한 항목을 즉시 영구 삭제합니다. 이 작업은 되돌릴 수 없습니다." confirmLabel="영구 삭제" busy={busy === 'bulk'} onClose={() => setBulkPurge(false)} onConfirm={purgeSelected}/> : null}
  </section>
}
