...
export default function TrashSection({ notify, onDataChanged }) {
  ...
  return <section className="settings-card">
    ...
    {loading ? <div className="trash-loading"><LoaderCircle className="spin"/> 불러오는 중…</div> : items.length ? shown.length ? <><div className="trash-list">{shown.map(item => { const key = `${item.table}-${item.id}`; return <div key={key}><span><small>{labels[item.table] || item.table}</small><strong>{item.title || item.content || '제목 없는 항목'}</strong><time>{item.deleted_at ? new Date(item.deleted_at).toLocaleString('ko-KR') : ''}</time></span><div><button className="secondary" disabled={busy === key} onClick={() => restore(item)}><RotateCcw/> 복원</button></div></div> })}</div><div className="trash-footer"><button className="danger-button" onClick={() => setCleanup(true)}><Trash2/> 30일 지난 항목 정리</button></div></> : <p className="empty-state">검색 조건에 맞는 항목이 없습니다.</p> : <p className="empty-state">휴지통이 비어 있습니다.</p>}
