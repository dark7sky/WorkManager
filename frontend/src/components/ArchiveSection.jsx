...
export default function ArchiveSection({ notify, onDataChanged, entity = 'task' }) {
  ...
  return <section className="settings-card">
    ...
    {loading ? <div className="trash-loading"><LoaderCircle className="spin"/> 불러오는 중…</div> : items.length ? <div className="trash-list">{items.map(item => <div key={item.id}><span><strong>{itemLabel(item)}</strong><time>{item.archived_at ? new Date(item.archived_at).toLocaleString('ko-KR') : ''}</time></span><div><button className="secondary" disabled={busy === item.id} onClick={() => unarchive(item)}><RotateCcw/> 보관 해제</button></div></div>)}</div> : <p className="empty-state">{labels.empty}</p>}
  </section>
}
