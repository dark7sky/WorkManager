...
export default function TaskForm({ task, tasks = [], onSave, onCancel, onDelete, onDirtyChange }) {
  ...
  return <form ref={formRef} className="form-grid" onChange={() => setDirty(true)} onSubmit={submit}>
    ...
    {task?.recurrence_group_id ? <label className="span-2"><input type="checkbox" checked={applyToSeries} onChange={e => setApplyToSeries(e.target.checked)}/> <span>이 업무와 이후 반복 업무에 모두 적용 (제목·설명·태그·색상 등)</span></label> : null}
