...
export default function TaskForm({ task, tasks = [], onSave, onCancel, onDelete, onDirtyChange }) {
  ...
  const createShareLink = async () => {
    ...
    try { const res = await api.shareTask(task.id, shareExpiryDays ? Number(shareExpiryDays) : undefined, sharePassword || undefined); setShareToken(res.public_token); setShareExpiresAt(res.public_token_expires_at || ''); setShareHasPassword(!!res.has_password) }
    catch (e) { setError(e.message) }
    finally { setShareBusy(false) }
  }
  const revokeShareLink = async () => {
    setShareBusy(true)
    try { await api.unshareTask(task.id); setShareToken(''); setShareCopied(false); setShareExpiresAt(''); setSharePassword(''); setShareHasPassword(false) }
    catch (e) { setError(e.message) }
    finally { setShareBusy(false) }
  }
  ...
  return <form ref={formRef} className="form-grid" onChange={() => setDirty(true)} onSubmit={submit}>
    ...
    {task?.recurrence_group_id ? <label className="span-2"><input type="checkbox" checked={applyToSeries} onChange={e => setApplyToSeries(e.target.checked)}/> <span>이 업무와 이후 반복 업무에 모두 적용 (제목·설명·태그·색상·시작/완료 시각 등)</span></label> : null}
